...
# Bump this version whenever chunking logic changes to invalidate stale caches
CHUNK_VERSION = "8"   # v8: NLI pre-filter + mandatory Groq verification
...
def load_chunks(doc_name: str) -> list | None:
    ...
    Returns None (cache miss) if chunks were stored by an older version.
    """
    try:
        collection = get_collection(doc_name)
        ...
        # Invalidate cache if chunking version has changed
        meta = collection.metadata or {}
        stored_version = meta.get("chunk_version", "0")
        if stored_version != CHUNK_VERSION:
            # Stale cache — delete and return None so doc gets re-chunked
            try:
                client = chromadb.PersistentClient(path=CHROMA_PATH)
                safe_name = "doc_" + hashlib.md5(doc_name.encode()).hexdigest()[:12]
