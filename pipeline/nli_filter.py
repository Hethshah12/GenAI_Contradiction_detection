...
def find_intra_chunk_contradictions(
    ...
    # Pre-filter: only score pairs with keyword overlap (1 shared word enough for same section)
    pairs = []
    pair_indices = []
    for i in range(len(statements)):
        for j in range(i + 1, len(statements)):
            if topical_overlap(statements[i], statements[j], min_shared=1):
    ...
    for rc in related_chunks:
        ...
        for i, ts in enumerate(target_stmts):
            for j, rs in enumerate(related_stmts):
                # Require 2+ shared keywords for cross-section
                if topical_overlap(ts, rs, min_shared=2):
                    pairs.append((ts, rs))
